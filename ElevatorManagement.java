...
public class ElevatorManagement {
	...
	public Elevator findElevator(ArrayList<Elevator> myElevators,
			int fromFloor, int numFloors, boolean callUp) {
		...
		for (int i = 0; i < availableElevators.size(); i++) {
			elevatorDistances[i] = availableElevators.get(i).currentFloor
					- fromFloor;
		...
		if (callUp) {
			for (int i = 0; i < numElevators; i++) {
				...
				// Update shortest distance only if the closest elevator below
				// rider and is going up
			...
			// If none are available, return the elevator with the least
			// requests
		...
		// Rider wants to go down
			...
			for (int i = 0; i < numElevators; i++) {
				...
				int currentDistance = elevatorDistances[i];
				int currentRequestSize = elevatorNumRequests[i];
				// Update shortest distance only if the closest elevator above
				// rider and is going down
				if (!currentElevator.goingUp && currentDistance >= 0) {
